...
using System.Linq;
using System.Text;
using Blackhawk.LanguageConverters.Json.CodeWriters;
using Blackhawk.Models.Extensions;
using Newtonsoft.Json;
using Newtonsoft.Json.Linq;
using Pluralize.NET;

namespace Blackhawk.LanguageConverters.Json
{
    public class JsonClassGenerator : IJsonClassGeneratorConfig
    {
        ...
        private Pluralizer pluralizationService = new Pluralizer();
        ...
        public void GenerateClasses()
        {
            ...
            using (var reader = new JsonTextReader(sr))
            {
                ...
                if (json is JArray)
                {
                    // We only take the first 500 an hope we have valid information to generate the classes
                    // Otherwise it can take a very long time. We hope that we have enough data in those 500 
                    examples = ((JArray)json).Cast<JObject>().Take(100).ToArray();  
                }
